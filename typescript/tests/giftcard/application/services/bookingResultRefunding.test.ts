...
import { RefundGiftCardPolicy } from '@/giftcard/domain/policies/refundGiftCardPolicy.js';
...
describe('BookingResultRefunding', () => {
  ...
  describe('booking results handling', () => {
    it('on rejected should restore balance', () => {
      ...
      service.handleBookingResults(bookingRejected(bookingId, card.id(), refundAmount));
    ...
    it('on confirmed should do nothing', () => {
      ...
      service.handleBookingResults(bookingConfirmed(bookingId, card.id(), amount));
    ...
    it('should fail if card does not exist', () => {
      ...
      expect(() =>
        service.handleBookingResults(bookingRejected(bookingId, nonExisting, amount))
      ).toThrow();
    });
  });
});
